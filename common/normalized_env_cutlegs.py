...
class CoppeliaSimEnv:
    ...
    __leg_names = ['_ML','_HL','_MR','_HR']
    __joint_names = ['/m1', '/m2', '/m3']  # ThC, CTr, FTi
    __foot_names = ['/foot_ML', '/foot_HL', 
                    '/foot_MR', '/foot_HR']
    __IMU_names = ['/IMU_robot', '/IMU_ref']
    __forcesensor_names = ['/forceSensor_ML', '/forceSensor_HL', 
                           '/forceSensor_MR', '/forceSensor_HR']

    __joint_handle = np.zeros((4, 3), dtype=int).astype(int)  # joint handle (leg l, joint j)
    __target_positions = np.zeros((4, 3), dtype=float).astype(float)  # joint target position (leg l, joint j)
    __initjoint_position = np.zeros((12, 1), dtype=float).astype(float)

    observation_space = np.zeros((1 + 3 + 12 + 4 + 4, ), dtype=float).astype(float)  # body orientation, joint angles, forces, foot trajectory
    action_space = np.zeros((12, ), dtype=float).astype(float)  # joint angles
    
    observation_space_high = np.array([
                        0.26309153, 0.14352329, 0.11150103, 0.20889758,
                        0.71234864, 0.6082702, -0.5482097, 1.1905043, 0.9968296, -0.7116102,
                        0.90482897, 0.1928462, 2.266345, -0.27362847, -0.13811275, 2.4670823,
                        14.140027, 13.357503, 15.725126, 9.445448,
                        0.19966786, 0.11533475, 0.24561436, 0.1978422
                        ])
    
    observation_space_low = np.array([
                        0.19126146, -0.03429405, -0.16010809, -0.08192597,
                        -0.7474498, -0.2489663, -2.2368977, 0.27859572, 0.20474741, -2.5910378,
                        -0.54582196, -0.50561714, 0.35649562, -1.2296814, -0.9988971, 0.48642993,
                        0., 0., 0., 0.,
                        0.00926453, 0.00918054, 0.00868974, 0.00918513
                        ])

    action_space_high = np.array([
                            0.71432644, 0.6131103, -0.5386828, 1.1927193, 1.0053458, -0.7053654, 
                            0.9115751, 0.20103599, 2.268618, -0.27113712, -0.13196065, 2.4745364
                        ]) 

    action_space_low = np.array([
                            -0.75536764, -0.2783127, -2.2406516, 0.27379432,  0.17258137, -2.5969138, 
                            -0.54989666, -0.5026367,  0.34887356, -1.2245036, -1.002201,  0.48189536
                        ])

    def __init__(self, port=23002, OnTimeStep=True):
        ...
        for leg in range(self.__joint_handle.shape[0]):
            for joint in range(self.__joint_handle.shape[1]):
                ...
                self.__joint_handle[leg, joint] = self.sim.getObject(
                    self.__joint_names[joint] + self.__leg_names[leg % 4]
                )
        ...
        self.set_robot_joint(np.zeros((12, 1)))
        self.update()
    ...
    def normalize_observation(self, observation):
        ...
        joint_angles = observation[:, 4:16]
        joint_angles_low = self.observation_space_low[4:16]
        joint_angles_high = self.observation_space_high[4:16]
        ...
        forces = observation[:, 16:20]
        forces_low = min(self.observation_space_low[16:20])
        forces_high = max(self.observation_space_high[16:20])
        ...
        foot_traj = observation[:, 20:24]
        foot_traj_low = min(self.observation_space_low[20:24])
        foot_traj_high = max(self.observation_space_high[20:24])
    ...
    def set_robot_joint(self, target_pos):
        target_pos = target_pos.reshape((4, 3))
        offset = self.__initjoint_position.reshape((4, 3))
